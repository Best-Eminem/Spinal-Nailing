...
def gaussian_radius(det_size, min_overlap=0.7):
    ...
    r3  = (b3 + sq3) / 2
    return min(r1, r2, r3)
def gaussian3D(shape, sigma=1, scale=1):
    #sigma = shape[0]/6
    s, m, n = [(ss - 1.) / 2. for ss in shape]
    #生成三个array
    z, y, x = np.ogrid[-s:s+1,-m:m+1,-n:n+1]

    h = scale * np.exp(-(x * x + y * y + z * z) / (2 * sigma * sigma))
    #让h中极小的值取0
    h[h < np.finfo(h.dtype).eps * h.max()] = 0
    return h

def draw_umich_gaussian(heatmap, center, radius, k=1,scale = 1):
    diameter = 2 * radius + 1 #直径
    gaussian = gaussian3D((diameter, diameter, diameter), sigma=diameter / 12, scale=scale) #
    radius = int(radius)
...
def draw_umich_gaussian_with_torch(heatmap, center, radius, k=1,scale = 1): 
    #diameter = 2 * radius + 1 #直径
    diameter = 25
    #gaussian = gaussian3D_with_torch((diameter, diameter, diameter), sigma=(2 * radius + 1) / 12, scale=scale) #
    #print(torch.max(gaussian))
    radius_tp = int(radius)
    z, y, x = int(center[0]), int(center[1]),int(center[2])

    slice, height, width = heatmap.shape[0:3]

    behind, front = min(z, radius_tp), min(slice - z, radius_tp + 1)
    left, right = min(x, radius_tp), min(width - x, radius_tp + 1)
    top, bottom = min(y, radius_tp), min(height - y, radius_tp + 1)
    if z<slice and y<height and x<width and front+behind>0 and bottom+top>0 and left+right>0:
        gaussian = gaussian3D_with_torch((front+behind, bottom+top, left+right), sigma=(2 * radius + 1) / 12, scale=scale) #
        heatmap[z - behind:z + front, y - top:y + bottom, x - left:x + right] = gaussian
    #if min(heatmap[z - behind:z + front, y - top:y + bottom, x - left:x + right].shape) > 0 and min(gaussian[radius - behind:min(radius + front,slice-z+radius), radius - top:radius + bottom, radius - left:radius + right].shape) > 0:  # TODO debug
        #heatmap[z - behind:z + front, y - top:y + bottom, x - left:x + right] = gaussian[radius - behind:min(radius + front,slice-z+radius), radius - top:radius + bottom, radius - left:radius + right]
...
def gaussian3D_with_torch(shape, sigma=1, scale=1):
    ...
    #z, y, x = np.ogrid[-s:s+1,-m:m+1,-n:n+1]
    grid = torch.meshgrid(z,y,x)
    grid_stack = torch.stack(grid,axis=3)
    grid_stack.reshape(int(s*2+1),int(m*2+1),int(n*2+1),3)

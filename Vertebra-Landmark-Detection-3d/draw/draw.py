...
from preprocess import xyz2irc_irc2xyz
...
def draw_by_matplotlib(img_series,pts):
    ...
    for landmark in pts:
        ...
        image_array_section = img_series[landmark[0]]
        axes[0].imshow(image_array_section, cmap = 'gray')
        #axes[0].plot(landmark[2],landmark[1],'or:')
        ax = axes[0]
        ax.scatter(landmark[2],landmark[1],s=80,c='red')

        image_array_section = img_series[:,:,landmark[2]]
        axes[1].imshow(image_array_section, cmap='gray')
        #axes[1].plot(landmark[1], landmark[0], 'or:')
        ax = axes[1]
        ax.scatter(landmark[1], landmark[0], s=80, c='red')
        plt.show()
        plt.close()
#

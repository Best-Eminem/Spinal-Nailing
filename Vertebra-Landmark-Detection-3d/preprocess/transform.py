...
class RotationSpine(object):
    def get(dim, angle):

        # rotate by individual angle in specific dimension
        current_angle = angle
        #print("旋转（°）变换（x,y,z）：",current_angle)
        radian = np.asarray(current_angle) * np.pi / 180
        t = sitk.AffineTransform(dim)

        #3D
        # rotate about x axis
        t.Rotate(2, 1, angle=radian)
        # rotate about y axis
        # t.Rotate(0, 2, angle=radian[1])
        # rotate about z axis
        # t.Rotate(0, 1, angle=radian[2])
